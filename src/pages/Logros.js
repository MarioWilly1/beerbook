...
const Logros = () => {
  ...
  if (loading) return <p style={{ padding: 24 }}>Cargando logros...</p>;

  const totalXPEarned = Object.values(unlockedData).reduce(
    (s, a) => s + (a.xp_awarded || 0), 0
  );
    ...
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "#666", margin: 0, fontSize: 14 }}>
          {unlocked.size}/{ACHIEVEMENTS.length} desbloqueados · +{totalXPEarned} XP ganados
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
        {ACHIEVEMENTS.map((ach) => {
          ...
          const date = data?.unlocked_at
            ? new Date(data.unlocked_at).toLocaleDateString("es-AR", { day: "2-digit", month: "short", year: "numeric" })
            : null;
          ...
          return (
            ...
            >
              <div style={{ fontSize: 34, marginBottom: 8, filter: isUnlocked ? "none" : "grayscale(1)" }}>
                {ach.emoji}
              </div>
